...
pub fn main() {
    App::new()
        ...
        .add_systems(
            Update,
            (move_directional_light, move_point_light),
        )
...
fn setup(
    ...
    // Add the spot light
    commands.spawn((
        Transform::from_xyz(-1.8, 3.9, -2.7)
...
fn move_point_light(
    ...
    mut objects: Query<(
        &mut Transform,
        &mut MoveBackAndForthHorizontally,
    )>,
) {
    for (mut transform, mut move_data) in objects.iter_mut()
    {
        ...
        translation.x +=
            move_data.speed * timer.delta_secs();
